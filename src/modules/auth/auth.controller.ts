...
import {
  ...
  DeleteAdminDocs,
  GetAllAdminsDocs,
} from './decorators/admin.decorators';
...
export class AuthController {
  ...
  async Refresh(@Request() req, @Res({ passthrough: true }) res: Response) {
    ...
    return new ResponseSuccess('Làm mới token thành công', {
  ...
  async forgotPassword(@Body() dto: ForgotPasswordDto) {
    ...
    return new ResponseSuccess(
      'Nếu email tồn tại, OTP đã được gửi đến hộp thư của bạn',
      {},
    );
  }
  ...
  async createAdmin(@Body() body: RegisterDto) {
    return await this.authService.createAdmin(body.email, body.password);
  }
  ...
  async deleteAdmin(
    ...
    return await this.authService.deleteAdmin(req.user.userId, userId);
  }
}
