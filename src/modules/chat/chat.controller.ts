...
export class ChatController {
  ...
  async sendMessage(
    ...
    const image_urls = files?.length
      ? await Promise.all(
          files.map((f) =>
            this.cloudinaryService.uploadImage(f).then((r) => r.secure_url),
          ),
        )
      : [];
  ...
  @GetConversationsDocs()
  @Roles(Role.COMPANY, Role.STUDENT)
  async getConversations(@Req() req, @Query() query: GetConversationsQueryDto) {
    const userId: number = req.user.userId;
  ...
  @MarkReadDocs()
  @Roles(Role.COMPANY, Role.STUDENT)
  async markRead(@Req() req, @Body() dto: MarkReadDto) {
    const userId: number = req.user.userId;
  ...
  @SetHiddenDocs()
  @Roles(Role.COMPANY, Role.STUDENT)
  async setHidden(@Req() req, @Body() dto: SetHiddenDto) {
    const userId: number = req.user.userId;
  ...
  @SetBlockedDocs()
  @Roles(Role.COMPANY, Role.STUDENT)
  async setBlocked(@Req() req, @Body() dto: SetBlockedDto) {
    const actorId: number = req.user.userId;

    // Service cần trả về cả targetUserId để gateway biết notify ai
    const { result, targetUserId } = await this.chatService.setBlocked(
      actorId,
      dto,
    );

    this.chatGateway.broadcastBlocked(
      actorId,
      targetUserId,
      dto.conversationId,
      dto.blocked,
