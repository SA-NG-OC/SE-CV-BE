...
export class ChatRepository implements IChatRepository {
  ...
  async validateSendContext(
    conversationId: number,
    senderId: number,
  ): Promise<ISendContext> {
  ...
  async hasAnyBlocked(conversationId: number): Promise<boolean> {
    const [row] = await this.db
      .select({
        participant_id: schema.conversation_participants.participant_id,
      })
      .from(schema.conversation_participants)
      .where(
        and(
  ...
  async createMessageAndUpdateConversation(
    ...
    return this.db.transaction(async (tx) => {
      const [saved] = await tx.insert(schema.messages).values(data).returning();
  ...
  async getConversationListForUser(
    ...
    if (role === 'student') {
      ...
      const [{ total }] = await this.db
        ...
        .innerJoin(
          schema.conversation_participants,
          eq(
            schema.conversation_participants.conversation_id,
            schema.conversations.conversation_id,
          ),
        )
        .innerJoin(
          schema.companies,
      ...
      const rows = await this.db
        ...
        .leftJoin(
          ...
          and(
            eq(
              partnerParticipants.conversation_id,
              schema.conversations.conversation_id,
            ),
    ...
    const conditions = [...baseConditions];
    if (search)
      conditions.push(ilike(schema.students.full_name, `%${search}%`));
    const where = and(...conditions);
