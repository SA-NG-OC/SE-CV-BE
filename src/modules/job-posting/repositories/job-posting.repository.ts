...
import {
  JobPostingDomain,
  JobPostingDomainError,
} from '../domain/job-posting.domain';
...
export class JobPostingRepository implements IJobPostingRepository {
  ...
  private async fetchSkillMap(
    jobIds: number[],
  ): Promise<Map<number, JobSkillItem[]>> {
  ...
  private async fetchApplicantCountMap(
    jobIds: number[],
  ...
  async getJobCategories(): Promise<CategoryItem[]> {
    return this.db
      ...
      .where(eq(schema.job_categories.is_active, true)) as Promise<
      CategoryItem[]
    >;
  }
  ...
  async findById(jobId: number): Promise<{
  ...
  async findRawForAdmin(
    ...
    const [rows, [{ total }]] = await Promise.all([
      this.db
        ...
        .where(whereClause)
        .orderBy(
          sql`${schema.job_postings.created_at} desc`,
          sql`${schema.job_postings.job_id} desc`,
        )
        .limit(limit)
        .offset(offset),
      this.db
  ...
  async findRawSavedJobs(
    ...
    const [rows, [{ total }]] = await Promise.all([
      this.db
        ...
        .where(whereClause)
        .orderBy(
          sql`${schema.saved_jobs.created_at} desc`,
          sql`${schema.job_postings.job_id} desc`,
        )
        .limit(limit)
        .offset(offset),
