...
export class JobPostingDomain {
  ...
  edit(dto: UpdateJobPostingDto): void {
    ...
    Object.assign(this.props, {
      ...
      salaryMin:
        dto.salaryMin !== undefined ? dto.salaryMin : this.props.salaryMin,
      salaryMax:
        dto.salaryMax !== undefined ? dto.salaryMax : this.props.salaryMax,
      salaryType: dto.salaryType ?? this.props.salaryType,
      isSalaryNegotiable:
        dto.isSalaryNegotiable ?? this.props.isSalaryNegotiable,
  ...
  getSalaryDisplay(): string {
    if (
      this.props.isSalaryNegotiable ||
      (this.props.salaryMin === null && this.props.salaryMax === null)
    ) {
      return 'Thỏa thuận';
    }
    const fmt = (n: number) =>
    ...
    return this.props.salaryMin !== null
      ? `Từ ${fmt(this.props.salaryMin)}`
      : `Đến ${fmt(this.props.salaryMax!)}`;
  }
  ...
  get positionLevel() {
    return this.props.positionLevel;
  }
  get numberOfPositions() {
    return this.props.numberOfPositions;
  }
  get salaryMin() {
  ...
  get isSalaryNegotiable() {
    return this.props.isSalaryNegotiable;
  }
  get isActive() {
  ...
  get applicationCount() {
    return this.props.applicationCount;
  }
  get approvedAt() {
  ...
  private static guardPositions(count?: number): void {
    ...
    if (!Number.isInteger(count) || count < 1)
      throw new JobPostingDomainError(
        'Số lượng vị trí phải là số nguyên dương',
      );
  }
}
