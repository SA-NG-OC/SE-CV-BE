...
export const GetJobCardCompanyDocs = () =>
  applyDecorators(
    ApiOperation({
      summary: 'Lấy danh sách tin tuyển dụng của công ty (dạng card)',
      description:
        'Trả về danh sách tin tuyển dụng có phân trang và bộ lọc (search, tag, city). Yêu cầu quyền Company và JWT token.',
    }),
    ApiBearerAuth(),
    ...
    ApiQuery({
      ...
      required: false,
      description:
        'Số lượng bản ghi trên mỗi trang (tối đa 50, mặc định là 10)',
    }),
    ApiQuery({
      name: 'search',
      type: String,
