...
export class EmbeddingScorer {
  ...
  async scoreJobsForStudent(
    ...
    if (!embedding) {
      this.logger.log(
        `Embedding missing for student ${studentId}. Generating now...`,
      );
      const student =
        await this.recommendationRepo.getStudentProfile(studentId);
      ...
      await this.embeddingRepo.upsertStudentEmbedding(
        studentId,
        embedding,
        contentHash,
      );
    ...
    const candidates = await this.embeddingRepo.findSimilarJobs(
      embedding,
      limit,
    );
  ...
  async scoreStudentsForJob(
    ...
    if (!embedding) {
      this.logger.log(
        `[embedding.scorer.ts] Embedding missing for job ${jobId}. Generating now...`,
      );
      ...
      await this.embeddingRepo.upsertJobEmbedding(
        jobId,
        embedding,
        contentHash,
      );
    ...
    const candidates = await this.embeddingRepo.findSimilarStudents(
      embedding,
      limit,
    );
