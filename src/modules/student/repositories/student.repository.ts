...
export class StudentRepository implements IStudentRepository {
  ...
  async findStudentWithMajor(
    studentId: number,
  ): Promise<StudentWithMajorRaw | null> {
    const [row] = await this.db
      ...
      .leftJoin(
        schema.majors,
        eq(schema.majors.major_id, schema.students.major_id),
      )
  ...
  async countApplicationsByStudent(
    studentId: number,
  ...
  async findStudentCards(
    ...
    if (years && years.length > 0) {
      const yearNumbers = years.filter(
    ...
    if (minGpa !== undefined)
      conditions.push(gte(schema.students.gpa, minGpa.toString()));
  ...
  async findStudentProfileByUserId(
    userId: number,
  ): Promise<StudentProfileRaw | null> {
    ...
    const [row] = await this.db
      ...
      .leftJoin(
        resumesSub,
        eq(schema.students.student_id, resumesSub.student_id),
      )
      .where(eq(schema.students.user_id, userId))
      .limit(1);
    return row ?? null;
