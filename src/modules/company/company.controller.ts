...
export class CompanyController {
  ...
  async getFollowedCompanies(
    ...
    return new ResponseSuccess(
      'Lấy danh sách công ty đã follow thành công',
      data,
    );
  }
  ...
  async getMyCompany(@Req() req) {
    const userId = req.user.userId;
    const company: CompanyResponse | null =
      await this.companyService.getMyCompany(userId);
  ...
  @Roles(Role.COMPANY)
  async updateCompanyBasic(@Req() req, @Body() dto: UpdateCompanyBasicDto) {
    const userId = req.user.userId;
    const company = await this.companyService.updateCompanyBasic(userId, dto);
  ...
  @Roles(Role.COMPANY)
  async updateContact(@Req() req, @Body() dto: UpdateCompanyContactDto) {
    const userId = req.user.userId;
    const data = await this.companyService.updateCompanyContact(userId, dto);
  ...
  @Roles(Role.COMPANY)
  async updateDetail(@Req() req, @Body() dto: UpdateCompanyDetailDto) {
    const userId = req.user.userId;
    const data = await this.companyService.updateCompanyDetail(userId, dto);

    return new ResponseSuccess(
      'Cập nhật thông tin chi tiết công ty thành công',
      {},
    );
  }
  ...
  async updateCompanyCover(
    ...
    await this.companyService.updateCompanyCover(userId, uploadRes.secure_url);
  ...
  async uploadOfficeImages(
    ...
    const images = await this.companyService.addOfficeImages(userId, imageUrls);
