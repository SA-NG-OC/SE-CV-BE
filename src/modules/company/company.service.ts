...
} from '../follow/repositories/follow-repository.interface';
...
export class CompanyService {
  constructor(
    ...
    private readonly followRepo: IFollowedCompanyRepository,
    ...
    try {
      ...
      if (!company) {
        throw new InternalServerErrorException(
          'Có lỗi khi tạo lập công ty, vui lòng thử lại sau!',
        );
      }
  ...
  async getCompanyById(
    ...
    const checkFollowed = await this.followRepo.checkFollowed(
      userId,
      companyId,
    );
  ...
  async deleteOfficeImage(userId: number, imageId: number) {
    ...
    const image = await this.companyRepo.findImageByIdAndCompany(
      imageId,
      company.companyId,
    );
    if (!image)
      throw new NotFoundException(
        'Không tìm thấy ảnh hoặc ảnh không thuộc về công ty này.',
      );
  ...
  async changeCompanyStatus(companyId: number, dto: ChangeCompanyStatusDto) {
    try {
      ...
      if (dto.status === 'APPROVED') {
        ...
        result = await this.companyRepo.rejectCompany(
          companyId,
          dto.admin_note ?? '',
        );
      } else if (dto.status === 'RESTRICTED') {
        result = await this.companyRepo.restrictCompany(
          companyId,
          dto.admin_note ?? '',
        );
      } else {
