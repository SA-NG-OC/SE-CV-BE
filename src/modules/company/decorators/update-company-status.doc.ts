...
export const UpdateCompanyStatusDocs = () =>
  applyDecorators(
    ApiBearerAuth(),
    ApiOperation({
      summary: 'Cập nhật trạng thái công ty (Admin)',
      description:
        'Admin duyệt, từ chối hoặc hạn chế công ty. Yêu cầu role ADMIN. Khi từ chối có thể thêm note đi kèm (Không bắt buộc).',
    }),
    ApiParam({
      name: 'id',
      type: Number,
      ...
      schema: {
        ...
        properties: {
          ...
          admin_note: {
            ...
            description:
              'Ghi chú của admin (bắt buộc khi REJECTED hoặc RESTRICTED)',
          },
        },
      },
    }),
