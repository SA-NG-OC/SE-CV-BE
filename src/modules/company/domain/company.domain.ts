...
export class CompanyDomain {
  ...
  get totalJobsPosted() {
    return this.props.totalJobsPosted;
  }
  get totalFollowers() {
    return this.props.totalFollowers;
  }
  get createdAt() {
  ...
  isRestricted(): boolean {
    return this.props.status === 'RESTRICTED';
  }
  isActive(): boolean {
  ...
  private static guardName(name: string): void {
    if (!name?.trim())
      throw new CompanyDomainError('Tên công ty không được để trống');
  }
