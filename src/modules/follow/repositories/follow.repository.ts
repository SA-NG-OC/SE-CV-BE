...
@Injectable()
export class FollowedCompanyRepository implements IFollowedCompanyRepository {
  constructor(
    @Inject(DATABASE_CONNECTION)
    private readonly db: NodePgDatabase<typeof schema>,
  ) {}

  async follow(studentId: number, companyId: number): Promise<void> {
    try {
      await this.db.insert(schema.followed_companies).values({
